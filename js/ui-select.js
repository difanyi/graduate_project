...
; + function($) {
    ...
    UI_select.prototype = {
        ...
        _setHtml: function() {
            ...
            this.val(this._opt.initValue||this.el.val());

            var _txt = this._list.children('li.selected').text();
            var _value = this._list.children('li.selected').data('value');
            this._input.text(_txt).attr({'title': _txt, 'key':_value});
        },
        ...
        _bindEvent: function() {
            ...
            _this._wrap.on({
                ...
                'click': function(e) {
                    ...
                    if (_this._isIE&&$(e.target).parents('.purchase-container').length==0) {//排除直接购药部分
                        e.stopPropagation();
        ...
        val: function(value) {
            ...
            this._input.html(_selectedLi.attr('title')).attr('title', _selectedLi.attr('title'));
            if (_val != value) {
