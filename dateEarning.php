...
<script type="text/JavaScript">
   ...
   alert('正在查找:'+a);
   location.href="dateEarning.php?a="+a;
	   }
...
var para = document.getElementById("h6").className = "nav-list nav-registration nav-check";  
    ...
    <div class="cloudClinic-main flt" id="cloudClinic-main" style="height: 531px;">
		...
		<a href="dateEarning.php" class="left-nav-dateEarning  cc-left-mark"><span class="ln-i-dateEarning"></span>效期提醒</a>
	</div>
...
if($lb==""||$lb=='全部'){
$all=mysql_num_rows(mysql_query("select * from drugs where Yp_XQ<'$time'"));	

}else{
$all=mysql_num_rows(mysql_query("select * from drugs where Yp_XQ<'$time' and Yp_LB='$lb'"));
}  
if($all==0){?>
...
if($lb==""||$lb=='全部'){
	$sql="select * from drugs where Yp_XQ<'$time' order by Yp_XQ limit {$offset},{$lenght}";

}else{
$sql="select * from drugs where Yp_XQ<'$time' and Yp_LB='$lb' order by Yp_XQ limit {$offset},{$lenght}";
}  
...
<div>
								...
								<td class="long-width" style="color:red;"><?php echo $row["Yp_XQ"];?></td>
								<td class="little-width"><?php echo $row["Yp_LB"];?></td>
								<td class="little-width"><?php echo $row["Yp_BZGG"];?></td>
								<td class="long-width"><?php echo $row["Yp_SCCJ"];?></td>
								<td class="little-width"><?php echo $row["Yp_KC"];?><?php echo $row["Yp_BZDW"];?></td>
		...
		<?php }else{?>
								<tr data-drug-id="231433">
								<td class="little-width"><?php echo $row["Yp_Name"];?> </td>
								<td class="long-width" style="color:orange;"><?php echo $row["Yp_XQ"];?></td>
								<td class="little-width"><?php echo $row["Yp_LB"];?></td>
								<td class="little-width"><?php echo $row["Yp_BZGG"];?></td>
								<td class="long-width"><?php echo $row["Yp_SCCJ"];?></td>
								<td class="little-width"><?php echo $row["Yp_KC"];?><?php echo $row["Yp_BZDW"];?></td>
...
<center><a href='dateEarning.php?page=1&&a=<?php echo $lb;?>''>首页</a>|
<a href='dateEarning.php?page=<?php echo $prepage;?>&&a=<?php echo $lb;?>' >上一页</a>|
第 <?php echo $page;?> 页&nbsp;
共 <?php echo $allpage;?> 页|
<a href='dateEarning.php?page=<?php echo $nextpage;?>&&a=<?php echo $lb;?>'>下一页</a>|
<a href='dateEarning.php?page=<?php echo $allpage;?>&&a=<?php echo $lb;?>'>末页</a></center>
</div>
                    </div>
                </div>
